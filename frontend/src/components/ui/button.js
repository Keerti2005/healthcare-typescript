...
const buttonStyles = tv({
    ...
    variants: {
        ...
        size: {
            "extra-small": "h-8 px-[calc(var(--spacing)*2.7)] text-xs/4 **:data-[slot=avatar]:*:size-3.5 **:data-[slot=avatar]:size-3.5 **:data-[slot=icon]:size-3 lg:text-[0.800rem]/4",
...
const Button = ({ className, intent, size, shape, ref, ...props }) => {
    return (_jsx(ButtonPrimitive, { ref: ref, ...props, className: composeRenderProps(className, (className, renderProps) => buttonStyles({
            ...renderProps,
            intent,
            size,
            shape,
